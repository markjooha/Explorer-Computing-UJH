...
LEGACY_FONT_DIR = BASE_DIR / "fonts"
...
PRETENDARD_FONT_FILES = {
    100: "Pretendard-Thin.otf",
    200: "Pretendard-ExtraLight.otf",
    300: "Pretendard-Light.otf",
...
def get_pretendard_font_dir():
    """배포 환경과 로컬 실행 환경에서 사용할 Pretendard 폰트 폴더를 찾습니다."""
    if STATIC_FONT_DIR.exists():
        return STATIC_FONT_DIR
    return LEGACY_FONT_DIR


def render_pretendard_web_font_style():
    """Streamlit 화면 전체에 저장소의 Pretendard 폰트 패밀리를 적용합니다."""
    font_dir = get_pretendard_font_dir()
    if font_dir != STATIC_FONT_DIR or not font_dir.exists():
        return

    font_faces = []
    for weight, filename in PRETENDARD_FONT_FILES.items():
        if (font_dir / filename).exists():
            font_faces.append(
                "@font-face {"
                "font-family: 'Pretendard';"
                f"src: url('app/static/fonts/{filename}') format('opentype');"
...
def configure_matplotlib_korean_font(matplotlib_pyplot):
    """저장소에 포함된 Pretendard 폰트를 matplotlib 그래프 기본 폰트로 설정합니다."""
    from matplotlib import font_manager

    font_dir = get_pretendard_font_dir()
    preferred_font = font_dir / "Pretendard-Regular.otf"

    if font_dir.exists():
        for font_path in sorted(font_dir.glob("Pretendard-*.otf")):
            font_manager.fontManager.addfont(str(font_path))

    if preferred_font.exists():
        font_name = font_manager.FontProperties(fname=str(preferred_font)).get_name()
        matplotlib_pyplot.rcParams["font.family"] = font_name
    else:
        matplotlib_pyplot.rcParams["font.family"] = [
            "Pretendard",
            "AppleGothic",
            "Malgun Gothic",
            "NanumGothic",
    ...
    matplotlib_pyplot.rcParams["axes.unicode_minus"] = False
...
def run_streamlit_app():
    ...
    render_pretendard_web_font_style()
